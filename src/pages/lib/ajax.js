        ...
        return myFetch(ajaxFetch, config.timeout, httpRequestId).then((res) => {
            ...
            if (error.message === ERROR.TIMEOUT) {
                ...
                if (error.message === "400") {
                    message.error(`调用api格式错误`)
                } else if (error.message === "401") {
                    message.error(`用户未认证或身份失效`)
                } else if (error.message === "403") {
                    message.error(`API缺少访问权限`)
                } else if (error.message === "500") {
                    message.error(`服务端API错误，请联系管理员`)
                } else if (error.message === ERROR.SYSTEM) {
